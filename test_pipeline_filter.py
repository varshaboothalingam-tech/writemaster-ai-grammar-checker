...
# Check pipeline filtering for specific errors
# Track which errors get filtered and why
for e in nlp_errors:
    ...
    if confidence < 0.70 or is_fp:
        print(f"FILTERED: {e.rule_id:25s} [{e.start:4d}:{e.end:4d}] '{e.original}' -> '{e.replacement}' conf={confidence:.2f} fp={is_fp} reason={reason}")
    # Also check validator
    is_valid, val_reason = p.validator.validate(e, clean_text)
    if not is_valid:
        print(f"REJECTED: {e.rule_id:25s} [{e.start:4d}:{e.end:4d}] '{e.original}' -> '{e.replacement}' reason={val_reason}")
