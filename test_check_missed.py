...
# Run all 50 sentences through pipeline and check missed ones
sentences = [
...
for i, s in enumerate(sentences):
    r = p.check(s)
    # Check specific previously-missed sentences
    idx = i + 1
    if idx in (15, 20, 35, 39, 40, 44, 45, 48):
        if r:
            rules = [x['rule_id'] for x in r]
            print(f"  {idx}: FOUND: {rules}")
        else:
            print(f"  {idx}: MISS (after processing {i} previous sentences)")
