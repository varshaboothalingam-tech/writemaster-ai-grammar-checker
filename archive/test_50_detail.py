"""Per-sentence: expected vs detected."""
import sys
...
for i, s in enumerate(sentences, 1):
    issues = p.check(s)
    if not issues:
        print(f"{i:2d}. MISS: {s[:80]}")
    else:
        rules = [(e['rule_id'], f"'{e['original']}'->'{e['replacement']}'") for e in issues]
        for rule, fix in rules:
            print(f"{i:2d}. HIT  {rule:30s} {fix}")
        # Show what's still wrong
        # Count expected - we just print detected
