...
print(f"NLP detector found {len(nlp_errors)} errors in full text:")
for e in sorted(nlp_errors, key=lambda x: x.start):
    ctx = clean_text[max(0,e.start-20):min(len(clean_text),e.end+20)].replace('\n',' ')
    print(f"  [{e.start:4d}] {e.rule_id:25s} '{e.original}' -> '{e.replacement}' conf={e.raw_confidence:.2f}")
    print(f"         ...{ctx}...")

# Now check: what errors do INDIVIDUAL sentences find?
...
for i, s in enumerate(sentences):
    doc2 = nlp(s)
    errors = det.detect(s, doc2)
    if errors:
        for e in errors:
            total += 1
            print(f"  Sent {i+1}: {e.rule_id:25s} '{e.original}' -> '{e.replacement}'")
print(f"\nIndividual sentence check: {total} errors found")
